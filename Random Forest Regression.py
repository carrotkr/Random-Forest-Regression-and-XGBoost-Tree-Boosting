...
mean, std = norm.fit(data_train['SalePrice'])
print('Mean: {:.3f} Standard Deviation: {:.3f}'.format(mean, std))
...
# Correlated with SalePrice > 0.70.
# Reference [seaborn.lmplot].
#   Plot data and regression model fits across a FacetGrid.
#   seaborn.pydata.org/generated/seaborn.lmplot.html
sns.lmplot(x='OverallQual', y='SalePrice', data=data_train)
sns.lmplot(x='GrLivArea', y='SalePrice', data=data_train)

# Reference [seaborn.boxplot].
#   Draw a box plot to show distributions with respect to categories.
#   seaborn.pydata.org/generated/seaborn.boxplot.html
plt.figure(figsize=(10, 8))
sns.boxplot(x='HouseStyle', y='SalePrice', data=data_train)
plt.show()
...
rfr_predict = rfr_predict.reshape(-1,1)

#%%
print(rfr_predict)
predict = std_data_Y.inverse_transform(rfr_predict)

predict = pd.DataFrame(predict, columns=['SalePrice'])

submission = pd.concat([data_test['Id'], predict], axis=1)
